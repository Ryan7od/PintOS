...
/* List of all threads thate are currently asleep */
static struct list sleepList;
static struct lock sleepLock;
...
/* Comparator for wake time in sleeping threads */
bool
...
void
sleepListAddElement (struct sleepingThread *t)
{
    lock_acquire(&sleepLock);
    list_insert_ordered(&sleepList, &t->sleepElem, wakeTimeSort, NULL);
    lock_release(&sleepLock);
}

/* Checks to wake top element in sleep list */
void
sleepListTryWake (void)
{
    while (!list_empty(&sleepList)) {
        struct sleepingThread *t = list_entry(list_front(&sleepList), struct sleepingThread, sleepElem);

        if (timer_ticks() < t->wakeTime) break;
